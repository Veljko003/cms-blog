...
global $ConnectingDB;
// Utilisation de valeur $AdminId pour sélectionner les informations nécessaires d'admins
$sql  = "SELECT `aname`, `username`, `aheadline`, `abio`, `aimage` FROM `admins` WHERE '$AdminId'";
...
if (strlen($AHeadline)>30) {
    ...
    if (!empty($_FILES["Image"]["name"])) {
      // Mettre à jour le profil séléctionné avec $AdminId avec des valeurs, inclus l'image et le nom
      $sql = "UPDATE `admins`(`aname`, `aheadline`, `abio`, `aimage`) SET ('$AName', '$AHeadline', '$ABio', '$Image') WHERE '$AdminId'";
    }else {
      // Mettre à jour le profil séléctionné avec $AdminId avec des valeurs, exclus l'image et le nom
      $sql = "UPDATE `admins`(`aheadline`, `abio`) SET ('$AHeadline', '$ABio') WHERE '$AdminId'";
    }
